...
with tf.Session() as sess:
    sess.run(init)
    lo= sess.run(blo,feed_dict={xi:X})
    print np.floor(lo+0.5)
'''
...
for i in range(len(X)):
    #print psy[i],fy[i],i
    if (psy[i]==fy[i]):# and (psy[i] in c12):
        ct+=1
    if psy[i] in c12:
	total+=1
    if (psy[i] in fy3[i]):#and(psy[i] in c12):
        ct3+=1
