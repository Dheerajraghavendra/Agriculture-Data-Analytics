...
y1=[]
y2=[]
strain = [0 for i in range(2)]
stest = [0 for i in range(2)]
for i in range(2):
    with open('metatdata/'+str(i)+'.txt','r') as f:
...
wts[1] = np.zeros(shape=(nh1,no))+2*np.random.random((nh1,no))-1
...
#wts[7] = np.zeros(shape=(nh7,no))+2*np.random.random((nh7,no))-1
...
#wts[11] = np.zeros(shape=(nh11,nh12))+2*np.random.random((nh11,nh12))-0.5
#wts[12] = np.zeros(shape=(nh12,no))+2*np.random.random((nh12,no))+0.5


bs = [[] for _ in range(nh+1)]
bs[0] = np.zeros(shape=(1,nh1))#+2*np.random.random((1,nh1))-1+1
bs[1] = np.zeros(shape=(1,no))#+2*np.random.random((1,nh2))-1+1
...
#bs[12] = np.zeros(shape=(1,no))#-1+2*np.random.random((1,no))-1+1
...
lo = tf.nn.softmax(tf.matmul(l1,w2)+b2)
...
with tf.Session() as sess:
    ...
    teval = [0 for i in range(2)]
    teval3 = [0 for i in range(2)]
    ...
    print act
    print clt3
    tsteval = [0 for i in range(2)]
    tsteval3 = [0 for i in range(2)]
